namespace MichMcb.CsExt.Test.Dates.DateTimeExtensions
{
	...
	public sealed class Deconstruction
	{
		...
		{
			{
				new DateTime(2020, 7, 14, 16, 24, 59, 123).Deconstruct(out int year, out int month, out int day, out int hour, out int minute, out int second, out int millis);
			...
			{
				(int year, int month, int day, int hour, int minute, int second, int millis) = new DateTime(2020, 7, 14, 16, 24, 59, 123);
				Assert.Equal(2020, year);
				Assert.Equal(7, month);
				Assert.Equal(14, day);
				Assert.Equal(16, hour);
		...
		{
			new DateTime(2020, 7, 14, 16, 24, 59, 123).GetTimeParts(out int hour, out int minute, out int second, out int millis);
			Assert.Equal(16, hour);
			Assert.Equal(24, minute);
			Assert.Equal(59, second);
			Assert.Equal(123, millis);
