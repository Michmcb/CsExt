namespace MichMcb.CsExt.Test.Dates.UtcDateTime
{
	...
	public static class Ctor
	{
		...
		public static void Millis()
		{
			...
			Assert.Equal(UtcDateTime.MaxTicks, new UtcDateTime(UtcDateTime.MaxTicks).Ticks);
			Assert.Throws<ArgumentOutOfRangeException>(() => new UtcDateTime(UtcDateTime.MaxTicks + 1));
		...
		public static void YearMonthDayHourMinuteSecondMillis()
		{
			...
			Assert.Equal(UtcDateTime.UnixEpochTicks, new UtcDateTime(1970, 1, 1, 0, 0, 0, 0).Ticks);
			Assert.Equal(UtcDateTime.MaxTicks, UtcDateTime.MaxValue.Ticks);
			// Micros and 100-nanosecond increments
			Assert.Equal(9999, UtcDateTime.MaxTicks - new UtcDateTime(9999, 12, 31, 23, 59, 59, 999).Ticks);
		}
	}
}
