namespace MichMcb.CsExt.Events
{
	...
	/// <summary>
	/// Invokes the registered event handlers in serial, waiting for each one to complete before moving onto the next one.
	/// </summary>
	/// <typeparam name="TArgs">The type of the event args.</typeparam>
	public sealed class SerialAsyncEvent<TArgs> : IAsyncEvent<TArgs>
	{
		private readonly List<Func<object?, TArgs, Task>> handlers;
		/// <summary>
		/// Creates a new event with no handlers.
		/// </summary>
		public SerialAsyncEvent()
		{
			handlers = new();
		}
		/// <summary>
		...
		/// <summary>
		/// Invokes all handlers. Handlers are executed in serial; this will wait for one handler to complete before moving on to the next.
		/// </summary>
